...
describe_ts <- function(data, ts, id, group = NULL, digits = 3, scale_min = NULL,
  ...
  icc_list <- list()
  for (var in ts){
    Y1_cen <- data[[var]] - stats::ave(data[[var]], data[[id]], FUN = function(x) mean(x, na.rm = TRUE))
    lvl1 <- var(Y1_cen, na.rm = TRUE)
    person_means <- tapply(data[[var]], data[[id]], mean, na.rm = TRUE)
    lvl2 <- var(person_means, na.rm = TRUE)

    icc_list[[var]] <- lvl2 / (lvl2+lvl1)
  }
