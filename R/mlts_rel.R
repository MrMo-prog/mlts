...
#' @param prob Numeric value between 0 and 1 specifying the upper probability
#'   bound used for the HDI in the \code{"RMU"} method.
...
#'     \item \code{rel_ci.lb}: Credibility interval lower bound
#'     \item \code{rel_ci.ub}: Credibility interval upper bound
#'   }
#' }
#'
#' @details
...
#' point estimates and uncertainty intervals for reliability.
#'
...
mlts_rel <- function(mltsfit,  method = c("EAP", "RMU"), prob = 0.95, seed = NULL){
  ...
  if (method== "EAP"){
    ...
    reliability_results <- sapply(u_pars, function(x) {
      var_EAP <- var(split_data[[x]]$mean, na.rm = TRUE)
