  ...
  for(pp in 1:N){
      ...
      for (t in (maxLag + 1):obs_id) {
         ...
         if (n_obs_exact > 0) {
            ...
            target += multi_normal_cholesky_lpdf(to_vector(obs_val[1:n_obs_exact]) | x_pred[obs_idx[1:n_obs_exact]], L_S);
         ...
         for (d in 1:D) {
            if (is_wcen[d] == 1) {
               int d_idx = D_cen_pos[d];
               if (is_cL[d, t] == 1) {
                  target += normal_lcdf(censL_val | x_upd[d_idx] + d_offset[d, t], sqrt(P_upd[d_idx, d_idx]));
               }
               if (is_cR[d, t] == 1) {
                  target += normal_lccdf(censR_val | x_upd[d_idx] + d_offset[d, t], sqrt(P_upd[d_idx, d_idx]));
               }
            }
         }
      }
